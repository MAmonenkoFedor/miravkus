...
import type { HomeHeroSettings } from "@/hooks/useProducts";

export function HeroBanner({ settings }: { settings?: HomeHeroSettings }) {
  ...
  const topBadge = settings?.topBadge || "🎁 Новая коллекция";
  const heroHeadline = settings?.headline || "Премиальные подарочные наборы";
  const heroHighlight = settings?.highlight || "подарочные наборы";
  const hasHighlight = heroHeadline.toLowerCase().includes(heroHighlight.toLowerCase());
  const headlinePrefix = hasHighlight
    ? heroHeadline.slice(0, heroHeadline.toLowerCase().indexOf(heroHighlight.toLowerCase())).trim()
    : heroHeadline;
  const highlightText = hasHighlight ? heroHeadline.slice(heroHeadline.length - heroHighlight.length) : "";
  const heroDescription =
    settings?.description ||
    "Изысканные азиатские сладости в элегантной упаковке. Идеальный подарок для ваших близких.";
  const primaryCtaText = settings?.primaryCtaText || "Выбрать подарок";
  const primaryCtaLink = settings?.primaryCtaLink || "/catalog?category=gift-sets";
  const secondaryCtaText = settings?.secondaryCtaText || "Смотреть каталог";
  const secondaryCtaLink = settings?.secondaryCtaLink || "/catalog";
  const feature1 = settings?.feature1 || "Бесплатная доставка";
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-center">
          ...
          <div className="text-white space-y-6 text-center lg:text-left">
            ...
            <div className="flex items-center justify-center lg:justify-start gap-6 pt-4 text-white/60 text-sm">
              ...
              <span className="flex items-center gap-2">
                <span className="text-lg">✓</span> {feature2}
              </span>
            </div>
          </div>
