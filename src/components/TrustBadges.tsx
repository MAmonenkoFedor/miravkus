...
import type { HomeTrustItem } from "@/hooks/useProducts";
...
export function TrustBadges({ items }: { items?: HomeTrustItem[] }) {
  const list = items && items.length > 0 ? items : features;
      ...
      <div className="container-custom">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {list.map((feature, index) => {
            const Icon =
              "icon" in feature && feature.icon && iconMap[feature.icon]
